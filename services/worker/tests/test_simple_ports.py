...
async def test_omitted_lindblad_reference_stays_weaker_when_models_disagree():
    ...
    assert planned.value is not None
    verification = planned.value.plan.verification_plan
    # Dropping the reference leaves nothing else in the verification plan and the
    # model normalizes that to None — pinned exactly, not as an either/or.
...
async def test_lindblad_audit_provider_failure_keeps_the_planner_range():
    """An audit outage must not let one unaudited extraction erase the Plan's range.

    exp(-0.7 * 1.3) ≈ 0.40 sits outside the declared range below, so this is the
    exact case where deleting the range would silently promote the single-model
    reference over the planner's REPLAN tripwire.
    """
    ports, llm, *_ = _ports()
    payload = _lindblad_reference_plan_payload()
    payload["verification_plan"].pop("exact_lindblad_reference")
    ...
    # Two responses only: the audit call exhausts the fake and raises — the
    # provider-failure path, distinct from a typed abstention.
    llm.texts = [
        json.dumps(payload),
        json.dumps(_lindblad_extraction_payload()),
    ]
    ...
    assert verification.exact_lindblad_reference is not None
    assert verification.reference_method == "independent_lindblad_extraction"
    assert planned.value.plan.success_criteria.expected_range == {"min": 0.9, "max": 1.0}
...
def test_missing_native_evidence_for_a_supported_profile_is_recorded_unavailable():
    """The gap must be a recorded check, not silence — and never a candidate defect."""
    payload = _plan_payload()
    payload.update(
        {
            "qubits_estimate": 12,
    ...
    execution = ExecutionEvidence(
        ...
        exit_code=0,
        duration_ms=1,
        result={"bloch_x": 0.0, "bloch_y": 0.0, "bloch_z": 1.0, "probability_one": 0.0},
        observation={"native_statevector_error": "statevector collection needs width <= 10"},
    )
    ...
    assert check["details"]["fault"] == "verifier_evidence"
    assert "width" in check["details"]["error"]

    unsupported = plan.model_copy(update={"expected_output_keys": ["counts"]})
    assert (
        simple_ports_module._native_result_consistency_check(
            unsupported.model_copy(
                update={
                    "success_criteria": unsupported.success_criteria.model_copy(
                        update={"primary_metric": "counts"}
                    )
                }
            ),
            execution,
...
def test_fully_specified_amplitude_damping_has_an_independent_scalar_oracle():
    ...
    check = simple_ports_module._amplitude_damping_reference_check(
        task, execution, list(execution.result)
    )
...
def test_fully_specified_qulacs_rotation_catches_a_self_consistent_sign_error():
    ...
    check = simple_ports_module._one_qubit_rotation_reference_check(
        task, execution, ["bloch_x", "bloch_y", "bloch_z", "probability_one"]
    )
...
def test_fully_specified_finite_qpe_has_a_displayed_order_distribution_oracle():
    ...
    check = simple_ports_module._finite_qpe_reference_check(task, execution, list(execution.result))
    ...
    # This constant was computed by the literal O(4^m) double sum this oracle
    # shipped with; the closed-form Dirichlet kernel must reproduce it.
...
def test_finite_qpe_oracle_skips_argmax_scalars_on_an_exact_tie():
    """Phase 0.25 on one counting qubit splits the mass evenly across both bins."""
    execution = ExecutionEvidence(
        execution_id=uuid4(),
        candidate_id=uuid4(),
        source_fingerprint="a" * 64,
    ...
    task = (
        "Implement noiseless Qiskit phase estimation for a diagonal 1-qubit target "
        "unitary. The prepared basis state has eigenvalue exp(2*pi*i*0.25). "
        "Use 1 counting qubits, controlled powers, and an inverse QFT."
    )

    check = simple_ports_module._finite_qpe_reference_check(task, execution, list(execution.result))

    assert check is not None and check["result"] == "pass"
    scores = check["details"]["scores"]
    assert "dominant_integer" not in scores
    assert "finite_phase_estimate" not in scores
    assert scores["dominant_probability"]["exact"] == pytest.approx(0.5)


def test_a_prompt_match_without_requested_profile_keys_is_no_oracle():
    """A prompt pattern alone must not demand RESULT keys nobody requested."""
    execution = ExecutionEvidence(
        execution_id=uuid4(),
        candidate_id=uuid4(),
        source_fingerprint="a" * 64,
    ...
    task = (
        "Implement noiseless Qiskit phase estimation for a diagonal 1-qubit target "
        "unitary. The prepared basis state has eigenvalue exp(2*pi*i*0.854455167). "
        "Use 4 counting qubits, controlled powers, and an inverse QFT."
    )

    assert simple_ports_module._finite_qpe_reference_check(task, execution, ["counts"]) is None
...
def test_repetition_qec_contract_keeps_requested_outputs_and_pins_exact_oracle():
    payload = _plan_payload()
    payload.update(
        {
            "expected_output_keys": [
    ...
    # The prompt gate proves worst_case_fidelity was requested, not that it is
    # the ONLY requested output — the other keys survive as obligations.
    assert reconciled.expected_output_keys == [
        "worst_case_fidelity",
        "fidelity_no_error",
        "fidelity_error_q0",
    ]
...
def test_single_model_lindblad_reference_is_graded_request_scoped_not_exact():
    """Without audit consensus a pass must not carry an _EXACT_SUCCESS_PROTOCOLS name."""
    exact_population = math.exp(-0.7 * 1.3)
    plan, execution = _exact_lindblad_plan_and_execution(excited_population=exact_population)
    plan = plan.model_copy(
        update={
            "verification_plan": plan.verification_plan.model_copy(
                update={"reference_method": "independent_lindblad_extraction"}
            )
        }
    )
    ...
    assert check["result"] == "pass"
    protocol = check["details"]["protocol"]
    assert protocol["name"] == "request_scoped_lindblad_evolution"
    assert protocol["reference_method"] == "independent_lindblad_extraction"
    assert protocol["name"] not in simple_ports_module._EXACT_SUCCESS_PROTOCOLS
