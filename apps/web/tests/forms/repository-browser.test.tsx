...
test("Atlas keeps filters open while typing and preserves the query in category links", async () => {
  ...
  try {
    ...
    const details = view.container.querySelector<HTMLDetailsElement>(".mj-facet-disclosure")!;
    fireEvent.click(details.querySelector("summary")!);
    assert.equal(details.open, true);
    const input = view.getByRole("searchbox");
    fireEvent.change(input, { target: { value: "grover" } });
    assert.equal(view.container.querySelector<HTMLDetailsElement>(".mj-facet-disclosure")?.open, true);
