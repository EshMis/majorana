...
const CLASSIQ_ALGORITHMS: ClassiqAlgorithm[] = [
  ...
  },
  {
    ...
    complexityBasis:
      'The abstract of arXiv:2101.10279, the only source read for this record, states: "We compare it with its classical analog for different annealing schedules and find a polynomial quantum advantage". It names no exponent, no big-O expression, and no other quantitative bound — not a qubit count, not a gate count, not a query count — for either algorithm; that clause is the entirety of the abstract\'s complexity content. The demonstration this record covers states no bound either.',
    caveat:
  ...
  },
  {
    ...
    complexityBasis:
      'The abstract of arXiv:2110.00968 states two separate scaling claims, each attached to a different resource. On qubits: "We require qubit numbers that grow linearly with the number of molecules". On a different pair of resources, scaling is derived but not given as an explicit function: "and derive scaling behaviour for the number of circuits and measurements required, which compare favourably to traditional variational quantum eigensolver methods." No exponent, big-O expression, or constant accompanies either claim, and compare favourably states a direction of comparison, not a bound: what the number of circuits or measurements actually is, as a function of system size, is not given. The demonstration this record covers states no complexity bound of its own. Those are the only sources read for this field.',
    caveat:
  ...
  },
  {
    ...
    complexity: "",
    complexityBasis:
      'The abstract of arXiv:0805.2741 states no algorithmic complexity, running time, gate count, or qubit count. Its one quantitative result is a physical efficiency figure for a specific molecular complex: "we demonstrate that for the FMO complex an effective interplay between free Hamiltonian and thermal fluctuations in the environment leads to a substantial increase in energy transfer efficiency from about 70% to 99%." That is a result about the transfer efficiency of the FMO complex itself, obtained from the paper\'s theoretical framework, not a bound on the cost of the generalized quantum walk or of any other algorithm; the abstract states no runtime, no query count, and no comparison of computational cost for the framework it develops. The demonstration this record covers states no complexity bound of its own. Those are the only sources read for this field, and the complexity field is left empty on purpose rather than filled with an efficiency figure that is not a complexity bound.',
    caveat:
  ...
  // and QAOA, and that similarity is not a citation.
  {
    ...
    complexityBasis:
      ...
      + " sixteen-link humanoid. The demonstration this record covers states no bound.",
    caveat:
    ...
    literature: [
      {
        ...
        url: "https://doi.org/10.1038/s41598-025-12109-0",
        relevance:
          "Primary source, and the demonstration's only reference: the reference implementation cites"
          + " this paper and nothing else. Read in full text rather than in abstract. It supplies the"
...
function classiqEntry(concept: ClassiqAlgorithm): PublicRepositoryEntry {
  ...
  return makeReferenceEntry({
    ...
    verification:
      "Literature record · the algorithm a published reference implementation demonstrates, checked"
      + " against that algorithm's primary reference on its arXiv abs page",
    verificationMethods: ["research_paper"],
    method:
      "Curation only. This record documents the algorithm a published reference implementation"
      + " demonstrates, not the demonstration itself: nothing in Leona runs Qmod. The primary"
      ...
      + " No circuit was constructed, simulated or run.",
    result:
    ...
    resources: [
      ...
      { label: "Reference implementation", value: concept.classiqName },
